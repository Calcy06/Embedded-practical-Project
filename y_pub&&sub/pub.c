                       ...
                       struct blob_attr *msg)
{
    ...
    blobmsg_add_u32(&b1, "co", data);   // 添加键值对："co": data 到缓冲区
    ubus_send_reply(ctx, req, b1.head); // 将打包好的co数据回复给客户端

    return 0;
}

// 定义 CO 传感器的 ubus 方法集合
// 用于向 ubus 总线提供接口供外部调用（如：ubus call co get_co）
static struct ubus_method co_methods[] = {
    {
        // ubus 调用方法名：执行 ubus call co get_co 时触发 */
        .name = "get_co",
...
// 定义gg传感器ubus对象类型
static struct ubus_object_type co_type = UBUS_OBJECT_TYPE("co", co_methods);
...
{
    // 推送 CO 数据
    publish_sensor_data(&co_obj, read_co, "co", &b1);

    // 推送音量数据
    publish_sensor_data(&volume_obj, read_volume, "volume", &b2);
...
int main()
{
    ...
    ubus_add_object(ctx, &co_obj); // 将 co 对象添加到 ubus
